...
public class LengendaryPickaxeItem extends ItemPickaxe implements IHasModel
{
    public LengendaryPickaxeItem()
    {
        super(ToolMaterialsInit.LEGENDARY_PICKAXE);
        setUnlocalizedName("legendary_pickaxe");
        setRegistryName("legendary_pickaxe");
        setCreativeTab(null);
        ...
        if(e instanceof EntityPlayer && !w.isRemote)
        {
            ...
            if(r.typeOfHit == RayTraceResult.Type.BLOCK)
            {
                ...
                if(side == EnumFacing.DOWN || side == EnumFacing.UP)
                {
                    ...
                    if(w.getBlockState(new BlockPos(x + 1, y, z - 1)).getBlock() != Blocks.OBSIDIAN)
                    {
                        this.destroyAndDropBlock(w, p, breaker, x + 1, y, z - 1);
                    }
                    if(w.getBlockState(new BlockPos(x + 1, y, z)).getBlock() != Blocks.OBSIDIAN)
                    {
                    ...
                    if(w.getBlockState(new BlockPos(x, y, z - 1)).getBlock() != Blocks.OBSIDIAN)
                    {
                ...
                {
                    if(w.getBlockState(new BlockPos(x + 1, y + 1, z)).getBlock() != Blocks.OBSIDIAN)
                    {
                        this.destroyAndDropBlock(w, p, breaker, x + 1, y + 1, z);
                    }
                    if(w.getBlockState(new BlockPos(x, y + 1, z)).getBlock() != Blocks.OBSIDIAN)
                    {
                ...
                {
                    if(w.getBlockState(new BlockPos(x, y + 1, z + 1)).getBlock() != Blocks.OBSIDIAN)
                    {
    ...
    private void destroyAndDropBlock(World w, EntityPlayer p, ItemStack breaker, int x, int y, int z)
    {
        ...
        if(w.getBlockState(pos).getBlockHardness(w, pos) >= 0 && w.getBlockState(pos).getBlock().getMaterial(w.getBlockState(pos).getBlock().getDefaultState()) == Material.GRASS)
        {
            w.getBlockState(pos).getBlock().harvestBlock(w, p, pos, w.getBlockState(pos), w.getTileEntity(pos), breaker);
            w.setBlockToAir(pos);
        }

        if(w.getBlockState(pos).getBlockHardness(w, pos) >= 0 && w.getBlockState(pos).getBlock().getMaterial(w.getBlockState(pos).getBlock().getDefaultState()) == Material.GROUND)
        {
            w.getBlockState(pos).getBlock().harvestBlock(w, p, pos, w.getBlockState(pos), w.getTileEntity(pos), breaker);
            w.setBlockToAir(pos);
        }
